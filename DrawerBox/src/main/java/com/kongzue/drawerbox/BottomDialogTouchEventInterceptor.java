...
public class BottomDialogTouchEventInterceptor {
    ...
    public BottomDialogTouchEventInterceptor(DrawerBoxDialog me, DrawerBoxDialog.DialogImpl impl) {
        refresh(me, impl);
    }
    
    public void refresh(final DrawerBoxDialog me, final DrawerBoxDialog.DialogImpl impl) {
        if (me == null || impl == null || impl.bkg == null || impl.scrollView == null) {
            return;
        }
        /**
         * DrawerBoxDialog 触控事件说明：
         * bkg 将拦截并接管所有触控操作。
         * DrawerBoxDialog 的启动方式依据是内容布局高度是否大于可显示安全区域的高度。
                        ...
                        case MotionEvent.ACTION_CANCEL:
                            ...
                            if (bkgOldY == 0) {
                                if (impl.boxBkg.getY() > impl.bkgEnterAimY + dip2px(35)) {
                                    ...
                                    enterAnim.start();
                                    impl.boxRoot.setInterceptBack(true);
                                    impl.boxRoot.setClickable(true);
                                }
                            } else {
